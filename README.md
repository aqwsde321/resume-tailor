...
- Docker 실행: 다른 사용자에게 공유할 때 권장. GitHub Actions가 Docker Hub `qrqr/resume-tailor` 이미지를 갱신하고, 사용자는 저장소를 clone한 뒤 `docker compose pull`로 바로 실행할 수 있습니다.
- 로컬 실행: 개발자가 Node.js와 Codex CLI를 직접 설치해서 실행합니다.

### 1.1 Docker 실행

가장 간단한 실행 방식입니다. `docker-compose.yml`은 기본적으로 `qrqr/resume-tailor:latest`를 내려받아 실행합니다.
...
- 로그인 정보는 `codex-home` Docker volume에 저장됩니다.
...
소스를 직접 빌드해 검증하고 싶을 때만 아래처럼 실행합니다.
...
- `RESUME_TAILOR_IMAGE`: Docker 실행 시 사용할 이미지 경로를 바꾸고 싶을 때 지정
