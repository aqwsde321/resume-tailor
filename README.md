...
- `POST /api/resume/fetch-url`: 이력서 URL 본문 추출
- `POST /api/resume/stream`: `resume-to-json` + SSE 로그
- `POST /api/company`: `company-to-json`
- `POST /api/company/fetch-url`: 채용공고 URL 본문 추출
...
- 공고 URL 불러오기는 사이트 구조에 따라 정확도가 달라질 수 있습니다. 현재 처리 방식은 [채용공고 불러오기 가이드](./docs/COMPANY_FETCH_GUIDE.md)를 참고하세요.
...
세부 문서 목록과 권장 읽기 순서는 [문서 인덱스](./docs/README.md)를 기준으로 유지합니다.
