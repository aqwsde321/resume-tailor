...
const PROFILE_IMAGE_BUFFER = readFileSync(
...
test("PDF 단계에서 프로필 이미지를 올리면 draft와 export 요청에 함께 반영된다", async ({
  page
}) => {
  await completeIntroFlow(page);
  ...
  const headerModal = page.locator(".pdf-editor-modal");
  await headerModal.locator("input[type='file']").setInputFiles({
    name: "profile.png",
    mimeType: "image/png",
    buffer: PROFILE_IMAGE_BUFFER
  });

  await expect(headerModal.getByAltText("프로필 이미지 미리보기")).toBeVisible();
  await expect(headerModal.getByLabel("PDF에 표시")).toBeChecked();
  await headerModal.getByRole("button", { name: "닫기" }).click();
  ...
  expect(
    (lastPdfRequestBody as { resume?: { pdfProfileImageDataUrl?: string } }).resume
...
test("URL 기반으로 step 1~4를 진행하고 PDF export 요청까지 보낼 수 있다", async ({ page }) => {
  ...
  await page.getByLabel("사용자 지정 PDF HEX").fill("#111111");
  await page.getByRole("button", { name: "선택 완료" }).click();
  await expect(page.locator(".pdf-theme-trigger")).toContainText("Custom");
  ...
  await page.locator(".pdf-editor-chip").filter({ hasText: "About Me" }).click();
