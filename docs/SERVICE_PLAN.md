...
- 생성 결과(JSON/자기소개)를 사용자가 직접 수정/확정 가능하게 둔다.
...
공통:
- 상단 스텝 이동(`/resume`, `/company`, `/result`)
- 확정 상태 배지(이력서/채용공고)
...
M1. 소개문 품질 개선:
- generate-intro 스키마 확장(`matchPoints[]`, `confidence` 등)
- 톤/길이 옵션 UI

M2. 품질/신뢰성 보강:
- 통합 E2E 테스트(플로우 + stale 규칙)
- 오류 재시도 정책 튜닝
