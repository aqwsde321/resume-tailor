...
1. [~] 자기소개 생성 품질 개선
2. [~] 채용공고 구조화 정확도 개선
3. [~] 단계별 수정과 재생성 UX 정교화
4. [~] E2E 테스트와 회귀 방지 체계 추가
5. [ ] PDF 입력과 배포형 구조 검토

## 2. 왜 이 순서인가

- 현재 사용자 체감 품질은 `자기소개 결과`와 `채용공고 구조화 품질`에 가장 크게 좌우됩니다.
- UI 흐름은 이미 MVP 기준으로 사용 가능한 상태이므로, 다음 단계는 생성 정확도와 재생성 신뢰성을 끌어올리는 것이 맞습니다.
- 그 다음에 테스트와 E2E를 보강해야 이후 프롬프트와 스키마 수정이 잦아져도 회귀를 빠르게 잡을 수 있습니다.
...
- 붙여넣은 공고 텍스트가 지저분해도 핵심 필드가 안정적으로 분리되게 만듭니다.
...
- 프롬프트, 스키마, 상태 규칙을 자주 수정해도 회귀를 빠르게 잡습니다.
...
- 핵심 플로우 회귀가 CI 수준에서 바로 잡힙니다.

### P5. 입력 확장과 배포 검토

목표:

- MVP 바깥의 입력 형식과 배포 제약을 다음 단계에서 검토합니다.
