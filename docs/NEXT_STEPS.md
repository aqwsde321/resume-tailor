...
1. [~] 자기소개 생성 품질 개선
2. [ ] Typst 기반 PDF 내보내기와 이력서 프로필 스키마 확장
...
- 그 다음으로는 이미 생성한 자기소개를 실제 제출 가능한 산출물(PDF)로 바로 이어 주는 가치가 큽니다.
- `resume-typst` 템플릿과 Typst 빌드 경로가 이미 외부에 존재하고, 현재 환경에서도 Typst 컴파일이 가능하므로 이식 비용은 통제 가능한 편입니다.
- 다만 PDF 내보내기는 기존 `Resume` 스키마보다 더 풍부한 메타 필드를 요구하므로, 이 기능은 UI/스키마/템플릿/테스트를 같이 설계하는 순서가 맞습니다.
- CI는 들어왔지만, 새 PDF 출력 경로까지 붙기 전에 fixture와 fallback 규칙을 먼저 문서화해 두는 편이 회귀를 줄입니다.
...
### P2. Typst 기반 PDF 내보내기와 이력서 프로필 스키마 확장
...
- [ ] `headline`이 없으면 `intro.oneLineIntro -> resume.summary 첫 문장 -> desiredPosition` 순서로 대체
- [ ] `careerDurationText`가 없으면 `careerYears` 기반 기본 문구로 대체
- [ ] `contacts[]`가 비어 있으면 있는 값만 출력하고, 전부 비어 있으면 연락처 줄 전체를 숨김
- [ ] 프로젝트 `link`가 없으면 링크 영역을 숨김
- [ ] 프로젝트 `highlights[]`가 없으면 `description`만 출력
- [ ] `techGroups[]`가 없으면 `techStack`을 규칙 기반으로 자동 분류하고, 분류되지 않는 항목은 `Other` 또는 마지막 그룹으로 보냄
- [ ] `experience[].projects[]`가 없으면 경력 설명만 출력하고 하위 프로젝트 블록은 숨김

스킬 필요 여부:

- [ ] 1차 구현은 새 skill 없이 진행
...
- [x] 외부 프로젝트 `/Users/jino/study/project/resume-typst`에 Typst 템플릿과 JSON 예제가 존재
- [x] 현재 환경에서 `typst compile` 가능
- [x] 현재 앱은 `oneLineIntro`, `shortIntro`, `longIntro`까지 생성 가능

완료 기준:

- `/result`에서 소개글 생성 후 PDF를 바로 다운로드할 수 있습니다.
- 연락처, 링크, 하이라이트가 비어 있어도 PDF 생성이 깨지지 않습니다.
- 로컬 실행과 Docker 실행 모두 PDF 빌드 경로가 동작합니다.
...
- [ ] PDF export 변환기와 fallback 규칙 테스트 추가
...
- [ ] PDF용 추가 필드를 입력해도 1단계 폼이 과하게 무거워지지 않도록 접기/보조 입력 설계
