...
`/company` 단계의 일반 사용법은 [README](../README.md)에 두고, 이 문서는 `URL 불러오기`와 사이트별 예외 처리 기준만 정리한다.
...
직접 붙여넣기와 `txt` 업로드는 최종적으로 같은 `companyText` 분석 흐름으로 합쳐진다는 점만 참고로 다룬다.
...
URL 지원은 `공고 텍스트를 채우는 입력 방식` 중 하나로 동작한다.
...
- 추천 공고 데이터와 현재 공고 데이터가 섞여 있어 공통 파서가 잘못 집기 쉽다.
...
- 상세 본문은 `GI_Read_Comt_Ifrm` iframe 기준으로 읽는다.
...
이 사이트들은 전용 selector 우선순위를 적용하고, 별도 API 호출 체인은 두지 않는다.

테스트 기준:

- Wanted: `meta title/og:title + main` 본문만으로도 핵심 구간이 비교적 안정적으로 잡힌다.
- Jumpit: `meta title/og:title + main` 본문만으로도 직무/요건/우대/기술 스택이 비교적 안정적으로 잡힌다.
- Wanted와 Jumpit은 정적 HTML 추출과 품질 회귀 테스트를 기준으로 유지한다.
...
- macOS 로컬 실행은 Vision OCR을 사용한다.
- Docker/Linux 실행은 `tesseract` 기반 OCR을 사용한다.
- 이미지 품질과 레이아웃에 따라 macOS Vision과 Linux Tesseract의 결과 품질 차이가 날 수 있다.
- 이미지 품질이 너무 낮거나 장식 요소가 많으면 정확도가 떨어질 수 있다.

## 7. 실패 시 동작

자동 추출 결과가 충분하지 않으면 아래 안내로 fallback한다.
...
- 잘못된 다른 공고가 섞이지 않도록 현재 공고 기준의 일부 본문만 반환하는 쪽을 우선한다.
- 그래서 사이트 전용 경로가 있으면 공통 parser보다 우선 적용한다.
- 현재 실행 환경의 OCR 결과가 충분하지 않으면, 자동 추출 결과보다 더 구체적인 안내를 우선 표시한다.
