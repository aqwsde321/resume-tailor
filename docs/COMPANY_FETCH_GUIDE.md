...
URL 지원은 별도 분석기가 아니라, `공고 텍스트를 채우는 입력 방식`이 추가된 구조에 가깝다.
...
이 사이트들은 전용 selector 우선순위만 두고, 현재는 별도 API 호출 체인은 없다.
...
- 따라서 두 사이트는 사람인/잡코리아처럼 별도 상세 요청 체인보다, 정적 HTML 품질 회귀 테스트를 유지하는 편이 효율적이다.
...
자동 추출 결과가 충분히 강하지 않으면 아래 안내로 fallback한다.
...
- 잘못된 다른 공고를 섞어 넣는 것보다, 현재 공고의 일부만 정확히 주는 편이 낫다.
